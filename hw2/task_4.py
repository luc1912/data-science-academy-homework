    ...
    if os.path.exists(directory):
        for file in os.listdir(directory):
            ...
            if file.endswith('.csv') and not file.startswith('._') and file not in filter:
                ...
                df = pd.read_csv(file_path, header=None, names=all_cols, usecols=needed_cols, encoding='utf-8', low_memory=False)
                all_data.append(df)
    else:
...
def process_data():
    ...
    january_path = os.path.join(LOCAL_FOLDER_PATH, 'january.csv')
    if os.path.exists(january_path):
        logging.info(f"Processing {january_path}")
        january_data = pd.read_csv(january_path, header=None, names=all_cols, usecols=needed_cols, encoding='utf-8', low_memory=False)
    else:
        logging.warning("January data not found.")
        january_data = pd.DataFrame()

    # load data for February and March 
    february_data = load_month_data_from_directory(os.path.join(LOCAL_FOLDER_PATH, 'february'))
    march_data = load_month_data_from_directory(os.path.join(LOCAL_FOLDER_PATH, 'march'))

    # combine all data into a single DataFrame
    all_data = [january_data, february_data, march_data]
